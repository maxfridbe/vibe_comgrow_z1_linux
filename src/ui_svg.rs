...
pub fn render_svg_left_col<'a, 'render>(
    ...
    clay.with(&left_col, |clay_scope| {
        ...
        clay_scope.with(&svg_box, |clay| {
            clay.text("SVG Path Mode", clay_layout::text::TextConfig::new().font_size((20.0 * font_scale) as u16).color(Color::u_rgb(192, 132, 252)).end());
            
            if render_burn_btn(clay, "btn_load_svg_ui", "Load SVG File", state, 0.0, 0.0, mouse_pressed, clipboard, font_scale) {
            ...
            clay.with(&preview_box, |clay| {
                clay.text("SVG PREVIEW", clay_layout::text::TextConfig::new().font_size((14.0 * font_scale) as u16).color(Color::u_rgb(100, 116, 139)).end());
            });
        });
    });
}
