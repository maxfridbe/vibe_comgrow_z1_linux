...
pub fn render_image_controls<'a, 'render>(
    ...
    clay.with(&left_col, |clay_scope| {
        ...
        clay_scope.with(&boundary_box, |clay_scope| {
            ...
            clay_scope.with(&grid, |clay_scope| {
                ...
                clay_scope.with(&col1, |clay_scope| {
                    render_slider(clay_scope, "img_bound_x", "X Pos", bx, 0.0, 400.0, COLOR_SLIDER_X, state, |s, v| s.boundary_x = v, mouse_pos, mouse_down, scroll_y, arena, font_scale);
                    render_slider(clay_scope, "img_bound_w", "Width", bw, 1.0, 400.0, COLOR_SLIDER_W, state, |s, v| s.boundary_w = v, mouse_pos, mouse_down, scroll_y, arena, font_scale);
                });
                ...
                clay_scope.with(&col2, |clay_scope| {
                    render_slider(clay_scope, "img_bound_y", "Y Pos", by, 0.0, 400.0, COLOR_SLIDER_Y, state, |s, v| s.boundary_y = v, mouse_pos, mouse_down, scroll_y, arena, font_scale);
                    render_slider(clay_scope, "img_bound_h", "Height", bh, 1.0, 400.0, COLOR_SLIDER_H, state, |s, v| s.boundary_h = v, mouse_pos, mouse_down, scroll_y, arena, font_scale);
                });
            });
        });
        ...
        img_box.layout().width(grow!()).direction(LayoutDirection::TopToBottom).padding(Padding::all(12)).child_gap(12).end()
            .background_color(COLOR_BG_SECTION)
            .corner_radius().all(16.0 * font_scale).end();
        ...
        clay_scope.with(&controls_box, |clay_scope| {
            ...
            clay_scope.with(&grid, |clay_scope| {
                let mut col1 = Declaration::<Texture2D, ()>::new(); col1.layout().direction(LayoutDirection::TopToBottom).child_gap(8).end();
                clay_scope.with(&col1, |clay_scope| {
                    render_slider(clay_scope, "img_power", "Power", pwr, 0.0, 1000.0, COLOR_SLIDER_POWER, state, |s, v| s.power = v, mouse_pos, mouse_down, scroll_y, arena, font_scale);
                    render_slider(clay_scope, "img_scale", "Scale", scl, 0.1, 5.0, COLOR_SLIDER_STEP, state, |s, v| s.scale = v, mouse_pos, mouse_down, scroll_y, arena, font_scale);
                });
                let mut col2 = Declaration::<Texture2D, ()>::new(); col2.layout().direction(LayoutDirection::TopToBottom).child_gap(8).end();
                clay_scope.with(&col2, |clay_scope| {
                    render_slider(clay_scope, "img_speed", "Speed", spd, 10.0, 6000.0, COLOR_SLIDER_SPEED, state, |s, v| s.feed_rate = v, mouse_pos, mouse_down, scroll_y, arena, font_scale);
                    render_slider(clay_scope, "img_passes", "Passes", pas as f32, 1.0, 20.0, COLOR_SLIDER_PASSES, state, |s, v| s.passes = v as u32, mouse_pos, mouse_down, scroll_y, arena, font_scale);
                });
            });
        });
    });
