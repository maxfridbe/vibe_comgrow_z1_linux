...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    while !rl.window_should_close() {
        ...
        root_decl.id(clay_scope.id("root"))
            .layout().width(fixed!(render_width)).height(fixed!(render_height)).padding(Padding::all(6)).child_gap(12).direction(LayoutDirection::TopToBottom).end()
        ...
        clay_scope.with(&root_decl, |clay_scope| {
            ...
            header_decl.layout().width(grow!()).height(fixed!(40.0 * font_scale)).padding(Padding::all(6)).child_alignment(Alignment::new(LayoutAlignmentX::Left, LayoutAlignmentY::Center)).end()
            ...
            clay_scope.with(&header_decl, |clay_scope| {
                ...
                clay_scope.with(&title_group, |clay_scope| {
                    ...
                    icon_box.layout().padding(Padding::all(4)).end().background_color(COLOR_PRIMARY_HOVER).corner_radius().all(6.0 * font_scale).end();
                    clay_scope.with(&icon_box, |clay_scope| {
                        clay_scope.text(ICON_LASER, clay_layout::text::TextConfig::new().font_size((16.0 * font_scale) as u16).color(COLOR_TEXT_WHITE).end());
                    });
                    clay_scope.text("COMGROW Z1", clay_layout::text::TextConfig::new().font_size((16.0 * font_scale) as u16).color(COLOR_TEXT_WHITE).end());
                });
                ...
                settings_group.layout().child_gap(8).child_alignment(Alignment::new(LayoutAlignmentX::Right, LayoutAlignmentY::Center)).end();
                clay_scope.with(&settings_group, |clay_scope| {
                    ...
                    if port == "VIRTUAL" { port_bg = COLOR_ACCENT_PURPLE_VIRTUAL; port_text_color = COLOR_TEXT_WHITE; }

                    let mut input_box = Declaration::<Texture2D, ()>::new();
                    input_box.id(port_h_id).layout().padding(Padding::all(4)).child_gap(8).child_alignment(Alignment::new(LayoutAlignmentX::Left, LayoutAlignmentY::Center)).end().background_color(port_bg).corner_radius().all(6.0 * font_scale).end();
                    clay_scope.with(&input_box, |clay_scope| {
                        clay_scope.text(ICON_USB, clay_layout::text::TextConfig::new().font_size((14.0 * font_scale) as u16).color(if port == "VIRTUAL" { COLOR_TEXT_WHITE } else { COLOR_USB_ICON }).end());
                        clay_scope.text(arena.push(port), clay_layout::text::TextConfig::new().font_size((12.0 * font_scale) as u16).color(port_text_color).end());
                    });
                    let mut wattage_box = Declaration::<Texture2D, ()>::new();
                    wattage_box.layout().padding(Padding::all(4)).child_gap(8).child_alignment(Alignment::new(LayoutAlignmentX::Left, LayoutAlignmentY::Center)).end().background_color(COLOR_BG_DARK).corner_radius().all(6.0 * font_scale).end();
                    clay_scope.with(&wattage_box, |clay_scope| {
                        clay_scope.text(ICON_CPU, clay_layout::text::TextConfig::new().font_size((14.0 * font_scale) as u16).color(COLOR_CPU_ICON).end());
                        clay_scope.text(arena.push(wattage), clay_layout::text::TextConfig::new().font_size((12.0 * font_scale) as u16).color(COLOR_WATTAGE_TEXT).end());
                    });
                    ...
                    estop_h_btn.id(estop_h_id).layout().padding(Padding::new(8, 8, 4, 4)).end().background_color(estop_h_color).corner_radius().all(6.0 * font_scale).end();
                    clay_scope.with(&estop_h_btn, |clay_scope| {
                        clay_scope.text("E-STOP", clay_layout::text::TextConfig::new().font_size((12.0 * font_scale) as u16).color(COLOR_TEXT_WHITE).end());
                    });
                });
            });
            ...
            clay_scope.with(&content_area, |clay_scope| {
                ...
                clay_scope.with(&col1, |clay_scope| {
                    ...
                    canvas_box.id(clay_scope.id("canvas")).layout().width(grow!()).height(grow!()).end().background_color(COLOR_BG_SECTION).corner_radius().all(16.0 * font_scale).end();
                    ...
                    clay_scope.with(&label_box, |clay_scope| {
                        ...
                        clay_scope.text(arena.push(format!("V: X:{:.1} Y:{:.1}", vx, vy)), clay_layout::text::TextConfig::new().font_size((12.0 * font_scale) as u16).color(COLOR_USB_ICON).end());
                        clay_scope.text(arena.push(format!("M: X:{:.1} Y:{:.1}", mx, my)), clay_layout::text::TextConfig::new().font_size((12.0 * font_scale) as u16).color(COLOR_TEXT_MUTED).end());
                        clay_scope.text(arena.push(format!("Status: {}", mstate)), clay_layout::text::TextConfig::new().font_size((14.0 * font_scale) as u16).color(COLOR_SUCCESS).end());
                ...
                clay_scope.with(&col2_scroll, |clay_scope| {
                    match current_tab {
                        UITab::Manual => {
                            ui_manual::render_manual_left_subcol(clay_scope, &state, &sections, mouse_pressed, &mut clipboard, &arena, font_scale);
            ...
            clay_scope.with(&bottom_area, |clay_scope| {
                ...
                log_box.id(serial_id_node).layout().width(grow!()).height(grow!()).padding(Padding::all(12)).direction(LayoutDirection::TopToBottom).child_gap(4).end()
                    .background_color(COLOR_BG_DARK).corner_radius().all(16.0 * font_scale).end().border().top((2.0 * font_scale) as u16).color(COLOR_ACCENT_PURPLE_LIGHT).end();
                ...
                clay_scope.with(&log_box, |clay_scope| {
                    ...
                    clay_scope.with(&title_row, |clay_scope| {
                        clay_scope.text("SERIAL LOG", clay_layout::text::TextConfig::new().font_size((12.0 * font_scale) as u16).color(COLOR_TEXT_DISABLED).end());
                    });
                    ...
                    clay_scope.with(&log_scroll, |clay_scope| {
                        for (i, log) in logs.iter().rev().take(100).enumerate() {
                            ...
                            if log.is_response { row.background_color(COLOR_TEXT_WHITE).corner_radius().all(4.0 * font_scale).end(); }
        ...
        for command in render_commands {
            match command.config {
                RenderCommandConfig::Rectangle(config) => {
                    let color = raylib::color::Color::new(config.color.r as u8, config.color.g as u8, config.color.b as u8, config.color.a as u8);
                    d.draw_rectangle_rounded(raylib::math::Rectangle::new(command.bounding_box.x, command.bounding_box.y, command.bounding_box.width, command.bounding_box.height), config.corner_radii.top_left / (command.bounding_box.width.min(command.bounding_box.height) / 2.0), 8, color);
                    let canvas_id = unsafe { clay_layout::bindings::Clay_GetElementId(clay_layout::bindings::Clay_String::from("canvas")).id };
                    if command.id == canvas_id {
                        canvas_rect_actual = raylib::math::Rectangle::new(command.bounding_box.x, command.bounding_box.y, command.bounding_box.width, command.bounding_box.height);
                    }
                }
        ...
        if canvas_rect_actual.width > 0.0 {
            ...
            if guard.boundary_enabled {
                ...
                let by = draw_area.y + draw_area.height - (guard.boundary_y / 400.0) * side - (guard.boundary_h / 400.0) * side;
                let bw = (guard.boundary_w / 400.0) * side;
                let bh = (guard.boundary_h / 400.0) * side;
                d.draw_rectangle_lines_ex(raylib::math::Rectangle::new(bx, by, bw, bh), 2.0, raylib::color::Color::new(52, 211, 153, 150));
            }
