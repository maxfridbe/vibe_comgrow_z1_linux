...
pub fn load_svg_data_as_gcode(
    ...
    let mut update_orig_bounds = |x: f32, y: f32| {
        if min_orig_x.is_none() || x < min_orig_x.unwrap() {
            min_orig_x = Some(x);
        }
        if max_orig_x.is_none() || x > max_orig_x.unwrap() {
            max_orig_x = Some(x);
        }
        if min_orig_y.is_none() || y < min_orig_y.unwrap() {
            min_orig_y = Some(y);
        }
        if max_orig_y.is_none() || y > max_orig_y.unwrap() {
            max_orig_y = Some(y);
        }
    };
    ...
    let mut update_bounds = |x: f32, y: f32| {
        if min_px.is_none() || x < min_px.unwrap() {
            min_px = Some(x);
        }
        if max_px.is_none() || x > max_px.unwrap() {
            max_px = Some(x);
        }
        if min_py.is_none() || y < min_py.unwrap() {
            min_py = Some(y);
        }
        if max_py.is_none() || y > max_py.unwrap() {
            max_py = Some(y);
        }
    };
        ...
        match op {
            Op::MoveTo(x, y) => {
                ...
                gcode.push_str(&format!(
                    ...
                    crate::gcode::move_xy(px, py),
                    crate::gcode::laser_on_dynamic_f(s_val as f32, f_val as f32)
                ));
                update_bounds(px, py);
            }
            Op::LineTo(x, y) => {
    ...
    Ok((
        gcode,
        min_px.unwrap_or(center_x),
        min_py.unwrap_or(center_y),
        max_px.unwrap_or(center_x),
        max_py.unwrap_or(center_y),
    ))
}
